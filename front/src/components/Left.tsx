...
const ActiveTitle = styled(TextTitle)<{ $active: boolean }>`
  color: ${({ $active }) =>
    $active ? '#1a73e8' : '#828282'}; // 활성화된 메뉴일 경우 색상 변경
`;

const IsOpenTitle = styled(TextTitle)<{ $isOpen: boolean }>`
  color: ${({ $isOpen }) =>
    $isOpen ? '#1a73e8' : '#828282'}; // 활성화된 메뉴일 경우 색상 변경
`;

const TextP = styled.p<{ $active: boolean }>`
  font-size: 16px;
  line-height: 30px;
  color: ${({ $active }) =>
    $active ? '#1a73e8' : '#828282'}; // 활성화된 메뉴일 경우 색상 변경
...
const StockFrame = styled.div<{ $isOpen: boolean }>`
  display: flex;
...
const StyledSVGall = styled.svg<{ $active: boolean }>`
  fill: ${({ $active }) =>
    $active ? '#1a73e8' : '#828282'}; // active 상태에 따라 색상 변경
  stroke: ${({ $active }) =>
    $active ? '#1a73e8' : '#828282'}; // active 상태에 따라 색상 변경
...
const IsOpenSVGfill = styled.svg<{ $isOpen: boolean }>`
  fill: ${({ $isOpen }) =>
    $isOpen ? '#1a73e8' : '#828282'}; // isOpen 상태에 따라 색상 변경
`;

const IsOpenSVGstroke = styled.svg<{ $isOpen: boolean }>`
  stroke: ${({ $isOpen }) =>
    $isOpen ? '#1a73e8' : '#828282'}; // isOpen 상태에 따라 색상 변경
`;
...
const Left: React.FC = () => {
  ...
  const [activeMenu, setActiveMenu] = useState(''); // 선택된 메뉴를 추적하는 상태
  const location = useLocation(); // 현재 경로를 가져옴

  // URL 변경 시 activeMenu와 드롭다운 상태를 업데이트
  useEffect(() => {
    setActiveMenu(location.pathname); // 현재 URL 경로를 activeMenu로 설정

    // 주식 관련 경로에 있을 경우 주식 드롭다운 열기
    if (
      ...
      setStockDropdown(false); // 다른 경로일 경우 닫기
    }

    // 뉴스 관련 경로에 있을 경우 주식 드롭다운 열기
    if (
      [
        '/news-main',
  ...
  const handleProtectedClick = (path: string) => {
    if (isLogin) {
      setActiveMenu(path); // 클릭한 메뉴의 이름을 상태로 설정
      navigate(path); // 메뉴에 해당하는 경로로 이동
    } else {
  ...
  const handleMenuClick = (menuName: string) => {
    setActiveMenu(menuName); // 클릭한 메뉴의 이름을 상태로 설정
    navigate(menuName); // 메뉴에 해당하는 경로로 이동
  };
  ...
  return (
    <LeftDiv>
      ...
      >
        ...
        <MenuSectionGG>
          <IsOpenTitle $isOpen={stockDropdown}>주식</IsOpenTitle>
          {!stockDropdown && (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="1.2rem"
      ...
      >
        <TextDiv
          ...
          onClick={() => handleMenuClick('/stock-main')}
        >
          <StyledSVG
            xmlns="http://www.w3.org/2000/svg"
            width="24"
        ...
        >
          ...
          <TextP $active={activeMenu === '/all-stock'}>전체 종목</TextP>
        </TextDiv>
        <TextDiv
          style={{ cursor: 'pointer' }}
          onClick={() => handleMenuClick('/section-stock')}
        >
          <StyledSVG
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            $active={activeMenu === '/section-stock'}
          >
          ...
          <TextP $active={activeMenu === '/section-stock'}>분야별 주식</TextP>
        </TextDiv>
        <TextDiv
        ...
        >
          ...
          <TextP $active={activeMenu === '/my-stock'}>나의 주식</TextP>
        </TextDiv>
      </DropdownMenu>
      ...
      >
        <TextDiv
          style={{ cursor: 'pointer', marginTop: '10px' }}
          onClick={() => handleMenuClick('/news-main')}
        >
          <StyledSVG
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            $active={activeMenu === '/news-main'}
          >
            <path d="M6 19H9V13H15V19H18V10L12 5.5L6 10V19ZM4 21V9L12 3L20 9V21H13V15H11V21H4Z" />
          </StyledSVG>
          <TextP $active={activeMenu === '/news-main'}>뉴스 메인</TextP>
        </TextDiv>
        <TextDiv
          style={{ cursor: 'pointer' }}
          onClick={() => handleMenuClick('/subnews-main/economic-news')}
        >
          <StyledSVGall
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            $active={activeMenu === '/subnews-main/economic-news'}
          >
            <path
        ...
        >
          <StyledSVG
            ...
            height="24"
            viewBox="0 0 24 24"
            $active={activeMenu === '/subnews-main/stock-news'}
          >
          ...
          <TextP $active={activeMenu === '/subnews-main/stock-news'}>
            종목 뉴스
          </TextP>
        </TextDiv>
        <TextDiv
      ...
      <MenuSection
        style={{ cursor: 'pointer' }}
        onClick={() => handleMenuClick('/ai-chat-bot')}
      >
        <StyledSVGstroke
          xmlns="http://www.w3.org/2000/svg"
          width="1.3rem"
